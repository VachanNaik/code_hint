...
class Interaction(nn.Sequential):
	def __init__(self, molecule_encoder, disease_encoder, protocol_encoder, 
		...
		self.feature_dim = self.molecule_encoder.embedding_size + self.disease_encoder.embedding_size + self.protocol_encoder.embedding_size + 1  # added by vachan naik, +1 for avg_affinity
	...
	def forward_encoder_2_interaction(self, molecule_embed, icd_embed, protocol_embed, avg_affinity_tensor=None): # added by vachan naik, avg_affinity_tensor=None
		...
		if avg_affinity_tensor is not None:# added by vacha naik, to handle the avg_affity
			encoder_embedding = torch.cat([encoder_embedding, avg_affinity_tensor], 1)  #added by vacha naik, to handle the avg_affity
	...
	# 	return output ### 32, 1
	# commented above originalk one..

	#added below funtion by vachan naik, to habdel the avg_affinity in forward  method..
	def forward(self, smiles_lst2, icdcode_lst3, criteria_lst, avg_affinity_tensor=None):
		molecule_embed, icd_embed, protocol_embed = self.forward_get_three_encoders(smiles_lst2, icdcode_lst3, criteria_lst)
		interaction_embedding = self.forward_encoder_2_interaction(molecule_embed, icd_embed, protocol_embed, avg_affinity_tensor)
		output = self.pred_nn(interaction_embedding)
		return output
    ...
    # above orginial one is commented , new testloader_to_lst is added below..
   
    #new addded below by vachan naik: to handel avg_affinity
	def testloader_to_lst(self, dataloader):
		nctid_lst, label_lst, smiles_lst2, icdcode_lst3, criteria_lst = [], [], [], [], []
		for nctid, label, smiles, icdcode, criteria, _ in dataloader:  # <-- added `_` for affinity_vec
			nctid_lst.extend(nctid)
			label_lst.extend([i.item() for i in label])
			smiles_lst2.extend(smiles)
			icdcode_lst3.extend(icdcode)
	...
	#   above orginial one is commented , new generate_predict(self, dataloader) is added below..
	...
	def generate_predict(self, dataloader):
		...
		# 	if len(batch) == 6:
		# 		nctid_lst, label_vec, smiles_lst2, icdcode_lst3, criteria_lst, affinity_vec = batch
		# 	else:
		# 		raise ValueError("Expected 6 elements per batch (including affinity_vec), got {}".format(len(batch)))
		for nctid_lst, label_vec, smiles_lst2, icdcode_lst3, criteria_lst, affinity_vec in dataloader:
			nctid_all.extend(nctid_lst)
			label_vec = label_vec.to(self.device)
			output = self.forward(smiles_lst2, icdcode_lst3, criteria_lst, affinity_vec).view(-1)
			loss = self.loss(output, label_vec.float())
			whole_loss += loss.item()
			predict_all.extend([i.item() for i in torch.sigmoid(output)])
			label_all.extend([i.item() for i in label_vec])
	...
	def learn(self, train_loader, valid_loader, test_loader):
		...
		for ep in tqdm(range(self.epoch)):
			# for nctid_lst, label_vec, smiles_lst2, icdcode_lst3, criteria_lst in train_loader: #commented this line , to add new modified line below..
			for nctid_lst, label_vec, smiles_lst2, icdcode_lst3, criteria_lst, affinity_vec in train_loader: # added and modified by vachan naik, to handle affinity_vec
				label_vec = label_vec.to(self.device)
				# output = self.forward(smiles_lst2, icdcode_lst3, criteria_lst).view(-1)  #### 32, 1 -> 32, ||  label_vec 32, # commented this line, added new modified line below.
				output = self.forward(smiles_lst2, icdcode_lst3, criteria_lst, affinity_vec=affinity_vec).view(-1) # added and modified by vachan naik, to handle affinity_vec
				loss = self.loss(output, label_vec.float())
...
class HINT_nograph(Interaction):
	...
	# def forward(self, smiles_lst2, icdcode_lst3, criteria_lst, if_gnn = False):###commented to add avg_affinty 
	def forward(self, smiles_lst2, icdcode_lst3, criteria_lst, avg_affinity_tensor=None, if_gnn=False): ####added by vachan naik to add avg_affinity
		...
		if if_gnn == False:
			...
			#Append avg_affinity if provided
			#### added by vachan naik.. to handel avg_affinty
			if avg_affinity_tensor is not None:
				embedding_lst.append(avg_affinity_tensor)
...
nn.ModuleList([ nn.ModuleList([nn.Linear(3,2) for j in range(5)] + [None]) for i in range(3)])
	...
	def generate_adj(self):        								
		...
		############ modified by vachan naik, to add avg_affinity in the lst variable and edge_lst
		lst = ["molecule", "disease", "criteria", 'INTERACTION', 'risk_disease', 'augment_interaction', 'A', 'D', 'M', 'E', 'T', 'PK', "avg_affinity", "final"]
		edge_lst = [("disease", "molecule"), ("disease", "criteria"), ("molecule", "criteria"), 
					...
					# ('augment_interaction', 'final'),('PK', 'final')] commented original
					##adedede ("PK", "avg_affinity"), ("avg_affinity", "final")
					("PK", "avg_affinity"), ("avg_affinity", "final"), ('augment_interaction', 'final')]
	...
	def forward(self, smiles_lst2, icdcode_lst3, criteria_lst, affinity_vec=None, return_attention_matrix = False): #addded by vachan to input avg_affinity
		embedding_lst = HINT_nograph.forward(self, smiles_lst2, icdcode_lst3, criteria_lst, avg_affinity_tensor=affinity_vec, if_gnn = True) ## modified by vachan naik this variable avg_affinity_tensor=affinity_vec
		...
		for i in range(batch_size):
			...
			# 🔹 Append avg_affinity if provided
			if affinity_vec is not None:
				avg_affinity_tensor = affinity_vec[i].view(1, -1)  # shape: [1, 1]
				node_feature_lst.append(avg_affinity_tensor)  # Append to node features
...
class FFNN(nn.Sequential):
	def __init__(self, molecule_dim, diseasecode_dim, 
					...
					lr = 3e-4, 
					weight_decay = 0, 
					use_affinity = True  #  Add flag , addded by vachan naik
					):
		...
		self.use_affinity = use_affinity  #  Save flag, addded by vachan naik
		#  Adjust input dimension if affinity is used
		input_dim = self.molecule_dim + self.diseasecode_dim + self.protocol_dim
		if self.use_affinity:
			input_dim += 1  # affinity is a single value per sample
    	
		# self.fc_dims = [self.molecule_dim + self.diseasecode_dim + self.protocol_dim, 2000, 1000, 200, 50, 1]
		self.fc_dims = [input_dim, 2000, 1000, 200, 50, 1]
	...
	# 	self = deepcopy(best_model)
	# 	auc_score, f1score, prauc_score, precision, recall, accuracy, predict_1_ratio, label_1_ratio = self.test(test_loader, return_loss = False, validloader = valid_loader)
	# commented above original one.. to modify the function for avg_affinty.. below add new one by vachan naik
	

	##added and modified by vachan naik, below function
	def learn(self, train_loader, valid_loader, test_loader):
		opt = torch.optim.Adam(self.parameters(), lr=self.lr, weight_decay=self.weight_decay)
		train_loss_record = [] 
		valid_loss = self.test(valid_loader, return_loss=True)
		...
		for ep in tqdm(range(self.epoch)):
			for nctid_lst, label_vec, smiles_lst2, icdcode_lst3, criteria_lst, affinity_vec in train_loader:  #  unpack affinity_vec
				X, _ = dataloader2Xy(nctid_lst, label_vec, smiles_lst2, icdcode_lst3, criteria_lst, self.global_icd)

				if self.use_affinity:
					X = torch.cat([X, affinity_vec], dim=1)  #  concatenate avg_affinity
	...
	#commented above functipon by vachan naik, added below..

	#modified and added new fun vctipn below by vachan naik to handel avg_affinity
	def generate_predict(self, dataloader):
		whole_loss = 0
		label_all, predict_all = [], []
		# for batch in dataloader:
		...
		# 	else:
		# 		raise ValueError("Expected 6 elements per batch (including affinity_vec), got {}".format(len(batch)))
		for nctid_lst, label_vec, smiles_lst2, icdcode_lst3, criteria_lst, affinity_vec in dataloader:  #  added affinity_vec
			X, _ = dataloader2Xy(nctid_lst, label_vec, smiles_lst2, icdcode_lst3, criteria_lst, self.global_icd)
			X = torch.cat([X, affinity_vec], dim=1)  #  append avg_affinity
