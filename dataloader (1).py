...
# added new by Vachan naik:
# purpose: to get prediction on new unseen new data , which label will be empty.
class Trial_Dataset_predict(data.Dataset):
    def __init__(self, nctid_lst, label_lst=None, smiles_lst=None, icdcode_lst=None, criteria_lst=None):
        self.nctid_lst = nctid_lst 
        self.label_lst = label_lst if label_lst is not None else [-1] * len(nctid_lst)  # Assign dummy labels
        self.smiles_lst = smiles_lst if smiles_lst is not None else []
        self.icdcode_lst = icdcode_lst if icdcode_lst is not None else []
        self.criteria_lst = criteria_lst if criteria_lst is not None else []
    ...
    def __getitem__(self, index):
        return (
            ...
            torch.tensor(self.label_lst[index], dtype=torch.float),  # Ensure tensor type even for dummy labels
...
# ADDED by Vachan Naik:
# purpose: to add new variable, to data loader .. avg_afinity, max_affinity


class Trial_Dataset_WithAffinity(data.Dataset):
    def __init__(self, nctid_lst, label_lst, smiles_lst, icdcode_lst, criteria_lst, affinity_lst):
        self.nctid_lst = nctid_lst 
        self.label_lst = label_lst 
        self.smiles_lst = smiles_lst 
        self.icdcode_lst = icdcode_lst 
        self.criteria_lst = criteria_lst 
        self.affinity_lst = affinity_lst

    def __len__(self):
        return len(self.nctid_lst)

    def __getitem__(self, index):
        return self.nctid_lst[index], self.label_lst[index], self.smiles_lst[index], self.icdcode_lst[index], self.criteria_lst[index], self.affinity_lst[index]
...
def csv_three_feature_2_dataloader(csvfile, shuffle, batch_size):
	...
	y_min = 2.0
	y_max = 10.0
	# affinity_lst = [float(row[13]) if len(row) > 13 and row[13] else 0.0 for row in rows] # New: avg_affinity
	affinity_lst = [(float(row[13]) - y_min) / (y_max - y_min) if len(row) > 13 and row[13] else 0.0 for row in rows]
	#########
	# dataset = Trial_Dataset(nctid_lst, label_lst, smiles_lst, icdcode_lst, criteria_lst) #coment this one 
	
	dataset = Trial_Dataset_WithAffinity(nctid_lst, label_lst, smiles_lst, icdcode_lst, criteria_lst, affinity_lst)  # #added avg_affinity
	########
	# data_loader = data.DataLoader(dataset, batch_size = batch_size, shuffle = shuffle, collate_fn = trial_collate_fn) #comment this to add new variable
	data_loader = data.DataLoader(dataset, batch_size = batch_size, shuffle = shuffle, collate_fn = trial_affinity_collate_fn) #add this to add afiinity lst
	return data_loader
...
def csv_three_feature_2_dataloader_for_prediction(csvfile, shuffle, batch_size):
    with open(csvfile, 'r') as f:
        rows = list(csv.reader(f, delimiter=','))[1:]  # Skip header

    # Extract relevant columns (make sure indices match your dataset structure)
    nctid_lst = [row[0] for row in rows]  # NCTID
    icdcode_lst = [row[6] for row in rows]  # ICD Codes
    smiles_lst = [row[8] for row in rows]  # SMILES (Molecular Structure)
    criteria_lst = [row[9] for row in rows]  # Eligibility Criteria

    # Set label_lst to `None`, ensuring that the dataset handles it properly
    dataset = Trial_Dataset_predict(nctid_lst, label_lst=None, smiles_lst=smiles_lst, icdcode_lst=icdcode_lst, criteria_lst=criteria_lst)
...
def trial_affinity_collate_fn(x):
    nctid_lst = [i[0] for i in x]     
    label_vec = default_collate([int(i[1]) for i in x])
    smiles_lst = [smiles_txt_to_lst(i[2]) for i in x]
    icdcode_lst = [icdcode_text_2_lst_of_lst(i[3]) for i in x]
    criteria_lst = [protocol2feature(i[4], sentence2vec) for i in x]
    affinity_vec = torch.tensor([float(i[5]) for i in x], dtype=torch.float32).unsqueeze(1)
    return [nctid_lst, label_vec, smiles_lst, icdcode_lst, criteria_lst, affinity_vec]
